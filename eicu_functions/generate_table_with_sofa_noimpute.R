generate.table.with.sofa.no.impute = function(start, end, n, data) {
    timestamps = sort(sample(start:end,n,replace=TRUE))
    hr = rep(NA, n)
    if (!is.null(data$hr)) {
        hr = eval.exact(timestamps,data$hr$timestamps,data$hr$values)
    }
    sbp = rep(NA, n)
    if (!is.null(data$sbp)) {
        sbp = eval.exact(timestamps,data$sbp$timestamps,data$sbp$values)
    }
    dbp = rep(NA, n)
    if (!is.null(data$dbp)) {
        dbp = eval.exact(timestamps,data$dbp$timestamps,data$dbp$values)
    }
    mbp = rep(NA, n)
    if (!is.null(data$mbp)) {
        mbp = eval.exact(timestamps,data$mbp$timestamps,data$mbp$values)
    }
    resp = rep(NA, n)
    if (!is.null(data$resp)) {
        resp = eval.exact(timestamps,data$resp$timestamps,data$resp$values)
    }
    temp = rep(NA, n)
    if (!is.null(data$temp)) {
        temp = eval.exact(timestamps,data$temp$timestamps,data$temp$values)
    }
    cvp = rep(NA, n)
    if (!is.null(data$cvp)) {
        cvp = eval.exact(timestamps,data$cvp$timestamps,data$cvp$values)
    }
    pao2 = rep(NA, n)
    if (!is.null(data$pao2)) {
        pao2 = eval.exact(timestamps,data$pao2$timestamps,data$pao2$values)
    }
    fio2 = rep(NA, n)
    if (!is.null(data$fio2)) {
        fio2 = eval.exact(timestamps,data$fio2$timestamps,data$fio2$values)
    }
    gcs = rep(NA, n)
    if (!is.null(data$gcs)&length(data$gcs$timestamps)>0) {
        gcs = eval.exact(timestamps,data$gcs$timestamps,data$gcs$values)
    }
    bili = rep(NA, n)
    if (!is.null(data$bili)) {
        bili = eval.exact(timestamps,data$bili$timestamps,data$bili$values)
    }
    platelets = rep(NA, n)
    if (!is.null(data$platelets)) {
        platelets = eval.exact(timestamps,data$platelets$timestamps,data$platelets$values)
    }
    creat = rep(NA, n)
    if (!is.null(data$creat)) {
        creat = eval.exact(timestamps,data$creat$timestamps,data$creat$values)
    }
    lactate = rep(NA, n)
    if (!is.null(data$lactate)) {
        lactate = eval.exact(timestamps,data$lactate$timestamps,data$lactate$values)
    }
    bun = rep(NA, n)
    if (!is.null(data$bun)) {
        bun = eval.exact(timestamps,data$bun$timestamps,data$bun$values)
    }
    ph = rep(NA, n)
    if (!is.null(data$ph)) {
        ph = eval.exact(timestamps,data$ph$timestamps,data$ph$values)
    }
    wbc = rep(NA, n)
    if (!is.null(data$wbc)) {
        wbc = eval.exact(timestamps,data$wbc$timestamps,data$wbc$values)
    }
    paco2 = rep(NA, n)
    if (!is.null(data$paco2)) {
        paco2 = eval.exact(timestamps,data$paco2$timestamps,data$paco2$values)
    }
    hgb = rep(NA, n)
    if (!is.null(data$hgb)) {
        hgb = eval.exact(timestamps,data$hgb$timestamps,data$hgb$values)
    }
    hct = rep(NA, n)
    if (!is.null(data$hct)) {
        hct = eval.exact(timestamps,data$hct$timestamps,data$hct$values)
    }
    potassium = rep(NA, n)
    if (!is.null(data$potassium)) {
        potassium = eval.exact(timestamps,data$potassium$timestamps,data$potassium$values)
    }
    urine = rep(NA, n)
    ...
    if (!is.null(data$pao2)) {
        ...
        sofa.pao2 = eval.exact(resp.sofa.timestamps,data$pao2$timestamps,data$pao2$values)
        sofa.fio2 = rep(NA, length(resp.sofa.timestamps))
        sofa.vent = rep(NA, length(resp.sofa.timestamps))

        if(!is.null(data$fio2)) {
            sofa.fio2 = eval.exact(resp.sofa.timestamps,data$fio2$timestamps,data$fio2$values)
        }
        if(!is.null(data$vent)) {
    ...
    if (!is.null(data$gcs)&length(data$gcs$timestamps)>0) {
        ...
        current.gcs = eval.exact(nervous.sofa.timestamps,data$gcs$timestamps,data$gcs$values)
    ...
    cardio.sofa = rep(0, n)
    vasopressors = rep(0, n)

    if (!is.null(data$map)) {
        current.map = eval.exact(cardio.sofa.timestamps,data$map$timestamps,data$map$values)
    ...
    if (!is.null(data$bili)) {
        ...
        current.bili = eval.exact(bili.timestamps,data$bili$timestamps,data$bili$values)
    ...
    if (!is.null(data$platelets)) {
        ...
        current.platelets = eval.exact(coag.timestamps,data$platelets$timestamps,data$platelets$values)
    ...
    if (!is.null(data$creat)) {
        ...
        current.creat = eval.exact(creat.timestamps,data$creat$timestamps,data$creat$values)
