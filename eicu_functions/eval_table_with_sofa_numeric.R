eval.table.with.sofa.numeric = function(timestamps, data, numeric) {
    # HR
    if (!is.null(data$hr)) {
        merged = merge.ts(data$hr$timestamps,data$hr$values,numeric$observationoffset,numeric$heartrate)
        hr = eval.carry.forward(timestamps,merged$timestamps,merged$values)
    } else {
        hr = eval.carry.forward(timestamps,numeric$observationoffset,numeric$heartrate)
    }
    # SBP
    if (!is.null(data$sbp)) {
        merged = merge.ts(data$sbp$timestamps,data$sbp$values,numeric$observationoffset,numeric$systemicsystolic)
        sbp = eval.carry.forward(timestamps,merged$timestamps,merged$values)
    } else {
        sbp = eval.carry.forward(timestamps,numeric$observationoffset,numeric$systemicsystolic)
    }
    # DBP
    if (!is.null(data$dbp)) {
        merged = merge.ts(data$dbp$timestamps,data$dbp$values,numeric$observationoffset,numeric$systemicdiastolic)
        dbp = eval.carry.forward(timestamps,merged$timestamps,merged$values)
    } else {
        dbp = eval.carry.forward(timestamps,numeric$observationoffset,numeric$systemicdiastolic)
    }
    # MBP
    if (!is.null(data$mbp)) {
        merged = merge.ts(data$mbp$timestamps,data$mbp$values,numeric$observationoffset,numeric$systemicmean)
        mbp = eval.carry.forward(timestamps,merged$timestamps,merged$values)
    } else {
        mbp = eval.carry.forward(timestamps,numeric$observationoffset,numeric$systemicmean)
    }
    # Resp
    if (!is.null(data$resp)) {
        merged = merge.ts(data$resp$timestamps,data$resp$values,numeric$observationoffset,numeric$respiration)
        resp = eval.carry.forward(timestamps,merged$timestamps,merged$values)
    } else {
        resp = eval.carry.forward(timestamps,numeric$observationoffset,numeric$respiration)
    }
    # Temperature
    if (!is.null(data$temp)) {
        merged = merge.ts(data$temp$timestamps,data$temp$values,numeric$observationoffset,numeric$temperature)
        temp = eval.carry.forward(timestamps,merged$timestamps,merged$values)
    } else {
        temp = eval.carry.forward(timestamps,numeric$observationoffset,numeric$temperature)
    }
    # CVP
    if (!is.null(data$cvp)) {
        merged = merge.ts(data$cvp$timestamps,data$cvp$values,numeric$observationoffset,numeric$cvp)
        cvp = eval.carry.forward(timestamps,merged$timestamps,merged$values)
    } else {
        cvp = eval.carry.forward(timestamps,numeric$observationoffset,numeric$cvp)
    }
    ...
    # SAO2
    sao2 = eval.carry.forward(timestamps, numeric$observationoffset, numeric$sao2)
    etco2 = eval.carry.forward(timestamps, numeric$observationoffset, numeric$etco2)
    icp = eval.carry.forward(timestamps, numeric$observationoffset, numeric$icp)
    pasys = eval.carry.forward(timestamps, numeric$observationoffset, numeric$pasystolic)
    padias = eval.carry.forward(timestamps, numeric$observationoffset, numeric$padiastolic)
    pamean = eval.carry.forward(timestamps, numeric$observationoffset, numeric$pamean)
    st1 = eval.carry.forward(timestamps, numeric$observationoffset, numeric$st1)
    st2 = eval.carry.forward(timestamps, numeric$observationoffset, numeric$st2)
    st3 = eval.carry.forward(timestamps, numeric$observationoffset, numeric$st3)

    result = data.frame(timestamps=timestamps,hr=hr,sbp=sbp,dbp=dbp,mbp=mbp,resp=resp,temp=temp,cvp=cvp,pao2=pao2,
        fio2=fio2,gcs=gcs,bili=bili,platelets=platelets,creat=creat,lactate=lactate,bun=bun,ph=ph,wbc=wbc,paco2=paco2,
        hgb=hgb,hct=hct,potassium=potassium,urine=urine,resp.sofa=resp.sofa,nervous.sofa=nervous.sofa,
        cardio.sofa=cardio.sofa,liver.sofa=liver.sofa,coag.sofa=coag.sofa,kidney.sofa=kidney.sofa,
        sao2=sao2,etco2=etco2,icp=icp,pasys=pasys,padias=padias,pamean=pamean,st1=st1,st2=st2,st3=st3)
    return(result)
}