generate.table.with.sofa.timestamps = function(start, end, n, data) {
    ...
    if (!is.null(data$urine)) {
        urine = eval.sum.in.past(timestamps,data$urine$timestamps,data$urine$values,hours(24))
    }
    ...
    if (!is.null(data$pao2)) {
        resp.sofa.timestamps = as_datetime(sort(unique(c(data$pao2$timestamps,data$fio2$timestamps))),tz="GMT")
        ...
        resp.sofa = eval.max.in.past(timestamps, resp.sofa.timestamps, resp.sofa.raw, hours(24), 0)
    }
    ...
    if (!is.null(data$gcs)&length(data$gcs$timestamps)>0) {
        nervous.sofa.timestamps = as_datetime(sort(unique(data$gcs$timestamps)),tz="GMT")
        ...
        nervous.sofa = eval.max.in.past(timestamps, nervous.sofa.timestamps, nervous.sofa.raw, hours(24), 0)
    }
    ...
    if (length(cardio.sofa.timestamps>0)) {
        cardio.sofa.timestamps = as_datetime(cardio.sofa.timestamps,tz="GMT")
    }
    ...
    cardio.sofa = eval.max.in.past(timestamps, timestamps, cardio.sofa.raw, hours(24))

    # Liver: bilirubin
    liver.sofa = rep(0, n)

    if (!is.null(data$bili)) {
        bili.timestamps = as_datetime(sort(unique(data$bili$timestamps)),tz="GMT")
        ...
        liver.sofa = eval.max.in.past(timestamps, bili.timestamps, liver.sofa.raw, hours(24), 0)
    }
    ...
    if (!is.null(data$platelets)) {
        coag.timestamps = as_datetime(sort(unique(data$platelets$timestamps)),tz="GMT")
        ...
        coag.sofa = eval.max.in.past(timestamps, coag.timestamps, coag.sofa.raw, hours(24), 0)
    }
    ...
    if (!is.null(data$creat)) {
        creat.timestamps = as_datetime(sort(unique(data$creat$timestamps)),tz="GMT")
