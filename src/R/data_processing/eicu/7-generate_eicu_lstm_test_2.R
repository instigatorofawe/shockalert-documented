...
source("src/R/functions/eicu/eval_table_with_sofa_timestamps_history.R")
...
nonsepsis.data = parLapply(cluster, 1:sum(!eicu.has.sepsis), function(x) eval.table.with.sofa.timestamps.history(eicu.sofa.scores[lengths>0][!eicu.has.sepsis][[x]]$timestamps,eicu.clinical.data[lengths>0][!eicu.has.sepsis][[x]],hours(1),12))
nonshock.data = parLapply(cluster, 1:sum(eicu.has.sepsis&!eicu.has.shock), function(x) eval.table.with.sofa.timestamps.history(eicu.sofa.scores[lengths>0][eicu.has.sepsis&!eicu.has.shock][[x]]$timestamps,eicu.clinical.data[lengths>0][eicu.has.sepsis&!eicu.has.shock][[x]],hours(1),12))
preshock.data = parLapply(cluster, 1:sum(eicu.has.shock), function(x) eval.table.with.sofa.timestamps.history(eicu.sofa.scores[lengths>0][eicu.has.shock][[x]]$timestamps[eicu.sofa.scores[lengths>0][eicu.has.shock][[x]]$timestamps<=eicu.shock.onsets[x]],eicu.clinical.data[lengths>0][eicu.has.shock][[x]],hours(1),12))
toc()
stopCluster(cluster)

save(nonsepsis.data,nonshock.data,preshock.data,file="data/eicu/eicu.lstm.test.rdata")
